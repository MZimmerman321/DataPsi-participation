...
tribble(
  ...
  ggplot(aes(x, y)) + 
  geom_text(aes(label = time), vjust = 0.5, hjust = 2) +
...
mtcars$cyc_fact<-as.factor(mtcars$cyc_fact
                           
mtcars %>% 
  
mutate(transmission = if_else(am == 0, "automatic", "manual")) %>% 
  ggplot(aes(fill = transmission, 
             y = (..count..)/sum(..count..), 
             x = cyc_fact)) +
  geom_bar(position = "dodge", color = "red", width = 0.3) +
...
```r
ggplot(aes(Hair, n, col = Hair, fill = Hair)) +
  geom_col() +
