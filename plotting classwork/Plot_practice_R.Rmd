...
gapminder %>% 
  ...
  geom_bar() +
  theme_bw()ggplot(gapminder, aes(gdpPercap, lifeExp))
```
...
```{r}
ggplot(gapminder, aes(gdpPercap, lifeExp)) + aes(colour = continent) +
    geom_point(alpha = 0.1) + 
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +  geom_point(alpha = 0.1)
```
...
ggplot(mauna, aes(month, conc)) + aes(colour = month)+
