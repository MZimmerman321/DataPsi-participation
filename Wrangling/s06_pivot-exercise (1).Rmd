...
```{r}
guest_long %>% 
  separate(name, into = c("First", "Last"), extra = "merge", fill = "left")
...
guest_long %>% 
  group_by(party) %>% 
  summarize(all_pending = all(attendance == (wedding = c("PENDING"))))
```
...
guest_long %>% 
  group_by(party) %>% 
  summarize(pending_wedding = all(wedding == "PENDING"))
```
