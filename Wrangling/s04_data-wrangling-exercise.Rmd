...
the `gapminder` data, in that order. changeing order in the ask will change output in call, make sur to use full variable name. 
...
#tilde is good because you can string multiple things together
#dont forget to assign it as anew varible if you want to save the results of what you did. 
```
...
replace bangladesh lifeexp with na
...
```{r}
gapminder %>%
  mutate(lifeExp = if_else(country == "Canada" & 
                             year == 1952, 70,
                             lifeExp))
```
